...
if [ $(arch) == "x86_64" ]; then
        ...
        docker build --pull --no-cache --build-arg KRUIZE_VERSION=${DOCKER_TAG} --build-arg OPENJ9V11_IMAGE=ppc64le/adoptopenjdk:11-jdk-openj9 --build-arg OS_IMAGE=ppc64le/ubuntu:focal -t ${KRUIZE_DOCKER_IMAGE}-$(arch) .
fi
